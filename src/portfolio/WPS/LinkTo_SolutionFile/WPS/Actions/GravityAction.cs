...
namespace WPS
{
    public class GravityAction : WPSAction
    {
        ...
        public GravityAction(GraphicsDevice GraphicsDevice, ContentManager Content, Vector3 gravity, float timeStep = 1)
        {
            this.TimeStep = timeStep;
            this.Gravity = gravity;
            this.GraphicsDevice = GraphicsDevice;

            effect = Content.Load<Effect>(Global_Variables_WPS.ContentActionEffects + "Gravity");
        }
        ...
        {
            effect.Parameters["Gravity"].SetValue(new Vector4(Gravity, 0));
        ...
        {
            Vector4 Grav = new Vector4(Gravity, 0);
            for (int i = 0; i < velocity.Length; i++)
            {
                velocity[i] += Grav * TimeStep;
            }
        }
    }
}
